...
import mobi.ioio.plotter.shapes.Target;
...
public class PlotterMainActivity extends IOIOActivity implements OnClickListener, OnJoystickMoveListener {
	private static final float[] HOME = { 375, 275 }; 
	...
	private class Looper extends BaseIOIOLooper {
		...
		private void startPlot() throws ConnectionLostException {
			...
			try {
				...
				plotter_.setMultiCurve(multiCurve);
		...
		public void plotImage(Float x, Float y, Float scale) throws ConnectionLostException, InterruptedException {
			MultiCurve drawing = new Target(x, y, 100, MM_PER_SEC);// = new
																	// BinaryImageMultiCurve(binaryImage,
																	// MM_PER_SEC,
																	// new
																	// float[]
																	// {x, y},
																	// scale,
																	// MIN_CURVE_PIXELS);
			plotter_.setMultiCurve(drawing);
			plot();
		}

		private void plot() throws ConnectionLostException, InterruptedException {
			while (true) {
				int time;
				synchronized (this) {
					time = plotter_.nextSegment(stepperStepCues_, stepperDirCues_, servoCue_);
				}
				if (time <= 0)
					break;
				sequencer_.push(cue_, time);
		...
		public void setManualSpeed(float x, float y) {
			manualSpeed_ = new float[] { x * MANUAL_MM_PER_SEC * MANUAL_SEC_PER_TICK , y * MANUAL_MM_PER_SEC * MANUAL_SEC_PER_TICK};
		}
	}
